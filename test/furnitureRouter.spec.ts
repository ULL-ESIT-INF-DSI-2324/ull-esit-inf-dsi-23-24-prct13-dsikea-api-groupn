...
describe("GET /furnitures", () => {
  it("Should find all furnitures", async () => {
    ...
    expect(
      response.body.map((furniture: IFurniture) => ({
        _id: furniture._id,
        name: furniture.name,
  ...
  it("Should return 200 and two furnitures with name chair ", async () => {
    ...
    ).to.deep.include.members([firstFurniture, thirdFurniture]);
  });
  ...
  it("Should return 200 if furniture name not found", async () => {
    ...
    ).to.deep.include.members([firstFurniture, secondFurniture]);
  });
});
...
describe("GET /furnitures/:id", () => {
  it("Should get a furniture by ID", async () => {
    const response = await request(app)
      .get("/furnitures/507f1f77bcf86cd799439011")
      .expect(200);
  ...
  it("Should return 404 if furniture ID not found", async () => {
    const response = await request(app)
      .get("/furnitures/663d72f1e8c597ba49ab8aa0")
      .expect(404);
...
describe("POST /furnitures", () => {
  it("Should successfully create a new furniture", async () => {
    ...
    const responseget = await request(app)
      .get("/furnitures?name=Sofa")
      .expect(200);

    expect(
      responseget.body.map((furniture: IFurniture) => ({
        name: furniture.name,
        description: furniture.description,
        material: furniture.material,
        color: furniture.color,
...
describe("PATCH /furnitures/:id", () => {
  it("Should successfully modify a furniture by ID", async () => {
    const response = await request(app)
      .patch("/furnitures/507f1f77bcf86cd799439011")
      .send({
        name: "Table",
  ...
  it("Should return 404 if furniture ID not found", async () => {
    const response = await request(app)
      .patch("/furnitures/997f1f77bcf86cd799439059")
      .send({
        name: "Table",
        description: "Glass table",
        material: "glass",
...
describe("PATCH /furnitures", () => {
  it("Should successfully modify a furniture by name and material", async () => {
    const response = await request(app)
      .patch("/furnitures?name=Table&material=wood")
      .send({
        name: "Table",
        description: "Glass table",
        material: "glass",
  ...
  it("Should return 400 if add quantity", async () => {
    const response = await request(app)
      .patch("/furnitures?material=acero")
      .send({
        name: "Table",
        description: "Glass table",
        material: "glass",
...
describe("DELETE /furnitures", () => {
  it("Should successfully delete a furniture by ID", async () => {
    const response = await request(app)
      .delete("/furnitures?name=Table")
      .expect(200);
    expect(response.body).to.eql({
      message: "Furniture deleted successfully",
    });
    const response_get_delete = await request(app)
      .get("/furnitures?name=Table")
      .expect(404);
  ...
  it("Should return 500 Multiple matching furniture", async () => {
    const response = await request(app)
      .delete("/furnitures?name=Chair")
      .expect(500);
    expect(response.body.error).to.eql(
      "Multiple matching furniture items have been found.",
    );
  });
  it("Should return 500 if furniture atribute is disntint", async () => {
    const response = await request(app)
      .delete("/furnitures?nombre=Chair")
      .expect(400);
    expect(response.body).to.eql({
      error:
        "Invalid search parameters, remember that the possible fields are: name, description, material and price",
...
describe("DELETE /furnitures/:id", () => {
  it("Should successfully delete a furniture by ID", async () => {
    const response = await request(app)
      .delete("/furnitures/507f1f77bcf86cd799439011")
      .expect(200);
    expect(response.body).to.eql({
      message: "Furniture deleted successfully",
    });
  ...
  it("Should return 404 if furniture ID not found", async () => {
    const response = await request(app)
      .delete("/furnitures/997f1f77bcf86cd799439059")
      .expect(404);
    expect(response.body.error).to.eql("Furniture not found");
  });
