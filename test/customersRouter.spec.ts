...
describe("GET /customer", () => {
  it("Should get a user by username with the dni in the query", async () => {
    const response = await request(app).get('/customers?dni=43549856D').expect(200);
  ...
  it("Should find all customers", async () => {
    ...
    expect(response.body.map((customer: ICustomer ) => ({
      name: customer.name,
  ...
  it("Not Should find a user by username beacuse not exist this DNI un the query", async () => {
    const response = await request(app).get("/customers?dni=43549856C").expect(404);
...
describe("GET /customers/:id", () => {
  it("Should find a customer 2 by id", async () => {
    const response = await request(app).get("/customers/507f1f77bcf86cd799439011").expect(200);
    expect(response.body).to.include({
      _id: "507f1f77bcf86cd799439011",
      name: "Alejandro Martinez",
      contact: "+34645678976",
  ...
  it("Not Should find a id not exist", async () => {
    const response = await request(app).get("/customers/663d5ae2b746172c48b08113").expect(404);
    expect(response.body.error).to.eql('Customer not found')
  });
});
...
describe("PATCH /customers", () => {
  ...
  it('Should successfully modify a customer', async () => {
    const response = await request(app).patch('/customers?dni=47540859E').send({
      name: "Alejandro Martinez",
      contact: "+34777345695",
      postalCode: "55555",
    }).expect(200);
  ...
  it('Not should successfully modify a customer not exist', async () => {
    const response = await request(app).patch('/customers?dni=45549856A').send({
      name: "Eduardo Segredo",
      contact: "+34666666677",
      postalCode: "34556",
    }).expect(404);
...
describe("PATCH /customers/:id", () => {
  ...
  it('Should successfully modify a customer', async () => {
    const response = await request(app).patch('/customers/507f1f77bcf86cd799439011').send({
  ...
  it('Not should  modify a customer, not found a customer', async () => {
    const response = await request(app).patch('/customers/997f1f77bcf86cd799439059').send({
      name: "Alejandro Martinez Gonzalez",
      email: "alemarti@hotmail.es",
      postalCode: "77777",
    }).expect(404);
...
describe("DELETE /customers/:id", () => {
  ...
  it('Should successfully delete a customer', async () => {
    const response = await request(app).delete('/customers/507f1f77bcf86cd799439011').expect(200);
  ...
  it('Not should delete a customer, not found a customer', async () => {
    const response = await request(app).patch('/customers/997f1f77bcf86cd799439059').expect(404);
    expect(response.body).to.eql(
      {
        error : "Customer not found"
      });
...
describe("DELETE /customers", () => {
  ...
  it('Should successfully delete a customer', async () => {
    const response = await request(app).delete('/customers?dni=47540859E').expect(200);
    expect(response.body).to.eql(
      {
        message: "Customer deleted"
      });
  ...
  it('Not should delete a customer, not found a customer', async () => {
    const response = await request(app).patch('/customers?dni=48888888A').expect(404);
    expect(response.body).to.eql(
      {
        error : "Customer not found"
      });
